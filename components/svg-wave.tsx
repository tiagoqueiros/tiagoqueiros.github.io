...
export default function SvgWave() {
    ...
    useEffect(() => {
        const startPaths = waveStartRef.current !== null ? (waveStartRef.current as any).querySelectorAll('path') : null;
        const endPaths = waveEndRef.current !== null  ? (waveEndRef.current as any).querySelectorAll('path') : null;

        if(startPaths === null) {
            return;
        }

        startPaths.forEach((path: any, index: number) => {
            anime({
                targets: path,
    ...
    return (
        <div className="wave-wrapper">
            <svg ref={waveStartRef} className="wave"  xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1920 1080">
                <g>
